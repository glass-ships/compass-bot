...
from compass.music.player import Timer
from compass.music.music_config import ErrorMessages, InfoMessages, MAX_SONG_PRELOAD
...
async def _play_check(itx: discord.Interaction) -> bool:
    ...
    if user_vc is None:
        await itx.followup.send(embed=discord.Embed(description=ErrorMessages.NO_USER_VC, color=COLORS().random()))
        return False
    ...
    if bot_vc is None:
        ...
        await itx.followup.send(
            embed=discord.Embed(description=ErrorMessages.WRONG_USER_VC(bot_vc), color=COLORS().random())
        )
        return False
...
class Music(commands.Cog):
    ...
    @play_check
    @app_commands.command(name="nowplaying", description="Show info about current song.")
    ...
    @app_commands.command(name="join", description="Move Compass to the user's VC")
    ...
    async def _move(self, itx: discord.Interaction, oldposition: int, newposition: int):
        player = music_utils.guild_player[itx.guild]
    ...
    async def _remove(self, itx: discord.Interaction, position: int):
        player = music_utils.guild_player[itx.guild]
